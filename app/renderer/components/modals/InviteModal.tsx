      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(520px,100%)', display: 'flex', flexDirection: 'column', gap: 14, padding: 22, background: '#eae9e9', borderRadius: 4, boxShadow: '0 12px 32px rgba(45,43,43,.22)', animation: 'wv-rise .16s ease-out' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              value={inviteQ}
              onChange={e => onInviteQChange(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); onAddInvite() } }}
          ...
          {invited.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
              {invited.map(h => {
                ...
                return (
                  <span key={h} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '4px 6px 4px 9px', background: '#e9f8ff', color: '#004961', borderRadius: 2, fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 12 }}>
                    {h}
                    <button onClick={() => onRemoveInvite(h)}
                      onMouseEnter={() => setXHover(true)}
                      onMouseLeave={() => setXHover(false)}
                      style={{ display: 'grid', placeItems: 'center', width: 18, height: 18, borderRadius: 2, color: '#006786', background: xHover ? 'rgba(0,136,176,.18)' : 'transparent', cursor: 'pointer' }}>
        ...
        {inviteRole === 'Guest' && (
          <div style={{ padding: '11px 12px', background: '#fff1f4', borderRadius: 2 }}>
            ...
            <div style={{ display: 'flex', gap: 7, marginTop: 9 }}>
              {EXPIRIES.map(ex => {
                const isActive = expiry === ex
                const [eHover, setEHover] = useState(false)
                return (
                  <button key={ex} onClick={() => onSetExpiry(ex)}
