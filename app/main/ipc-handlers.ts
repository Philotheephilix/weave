...
import { probeOnionReachable } from './onion-probe.js'
import { initCallOrchestrator, registerCallIpcHandlers } from './call-orchestrator.js'
...
export function registerIpcHandlers(
  ...
  // ── Peer onion reachability probe ───────────────────────────────────────────
  // Renderer calls this when opening a chat to show online/offline indicator.
  ipcMain.handle('weave:peer:reachable', async (_e, { peerLabel }: { peerLabel: string }) => {
    try {
      const resolved = await idMgr.resolveHandle(peerLabel).catch(() => null)
      if (!resolved?.onionAddress) return { reachable: false }
      const proxy = tor.getSocksProxy()
      const reachable = await probeOnionReachable(resolved.onionAddress, proxy)
      return { reachable, onionAddress: resolved.onionAddress }
    } catch {
      return { reachable: false }
    }
  })

  // ── WebRTC call signaling — Tor-first, Nostr fallback ──────────────────────
  ...
  ipcMain.handle('weave:call:signal', async (
    ...
    try {
      ...
      if (!resolved) return { ok: false, reason: 'peer not found' }

      const saved = loadIdentity() as { handle?: string } | null
      const senderLabel = saved?.handle ?? ''
      const payload = JSON.stringify({ from: senderLabel, signal })

      // Try Tor direct delivery first (both parties must be online)
      if (resolved.onionAddress) {
        try {
          const proxy = tor.getSocksProxy()
          await _sendOverSocks5(proxy.host, proxy.port, resolved.onionAddress, 80, payload, '/call/signal')
          return { ok: true, via: 'tor' }
        } catch {
      ...
      if (resolved.nostrPub) {
        ...
        if (!isPlaceholder) {
          const nostrPayload = JSON.stringify({ type: 'weave-call-signal', signal })
          await nostr.send(getIdentity().viewPriv, recipientNpub, nostrPayload)
          return { ok: true, via: 'nostr' }
        }
      }

      return { ok: false, reason: 'no reachable transport for peer' }
    } catch (err) {
